...
import OfferApiFilter from "../OfferApiFilter/OfferApiFilter";

// Dynamically import Lottie
const Lottie = dynamic(() => import("lottie-react"), { ssr: false });

interface OfferApiFilterModalProps {
  children?: ReactNode;
}

export default function OfferApiFilterModal({
  children,
}: OfferApiFilterModalProps) {
  return (
    <Modal.Root>
      <Modal.Trigger>{children}</Modal.Trigger>

      <Modal.Content className={`!min-w-[0] !w-fit`}>
        <div className="relative w-[80vw] bg-white rounded-md shadow-lg">
