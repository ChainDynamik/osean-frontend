...
import { DAO_ADDRESS, OSEAN_ADDRESS, USDT_ADDRESS } from "../../cost/addresses";
...
export default function TransferUSDT() {
  ...
  const transfer = async () => {
    try {
      ...
      const targets = [usdtContract?.getAddress()];
      const values = [0];
      const calldatas = [
        usdtContract?.encoder.encode("transfer", [wallet, toWei(amount)]),
      ];
        ...
        <Card style={{ maxWidth: "600px" }}>
          <Flex justify="space-between" alignItems="flex-start" width="100%">
            <Box flex="1" m={4}>
              <FormControl>
                <h3>Transfer USDT</h3>
                <Text>This is a prebuilt proposal to send a USDT amount from our treasury to an external Wallet.</Text>
              ...
              <FormControl>
                <FormLabel mt={3}>Amount in USDT</FormLabel>
                <Input
                  type="number"
                  value={amount}
